...
class Controller:
    '''
    Basic device adaptor for PI E-709.1C1L, compact (cost-optimized) single axis
    digital piezo controller for capacitive sensors (with monitoring functions).
    Many more commands are available and have not been implemented.
    '''
    def __init__(self,
                 which_port,            # COM port for controller
                 z_min_um,              # min range of connected piezo (um)
                 z_max_um,              # max range of connected piezo (um)
                 z_tol_um=0.25,         # tolerance on positional error (um)
                 name='E-709.1C1L',     # optional name
...
##    piezo._set_parameter('0x07000901', 0.00000000)
    ...
    piezo.move_um(400, relative=False)               # move somewhere
    piezo.set_analog_control_limits(0, 10, 0, 800)  # configure analog
